...
def rbf_math_forward(q, k, v, is_causal=True):
    ...
    """
    k_sq = k.float().pow(2).sum(dim=-1, keepdim=True)

    q_f32 = q.float()
    k_f32 = k.float()

    q_prime = torch.cat([q_f32, torch.ones_like(q_f32[..., :1])], dim=-1)
    k_prime = torch.cat([k_f32, -0.5 * k_sq], dim=-1)
    ...
    out_f32 = F.scaled_dot_product_attention(
        q_prime, k_prime, v.float(), is_causal=is_causal, scale=scale
    )
